...
#define BUTTON_BOTPMT 34
#define BUTTON_PLUGIN 35
...
void loop()
{
    ...
    if (!bGenerateFlag && digitalRead(BUTTON_BOTPMT) == HIGH) {
        ...
        szCozeTriggerPrompt="一個年輕女孩，手捧著鮮花，微笑的看著天空，水彩畫風格。圖上還有大大的「Good Morning」文字。";
    ...
    else if (!bGenerateFlag && digitalRead(BUTTON_PLUGIN) == HIGH) {
        bGenerateFlag=true;
        digitalWrite(2, HIGH);
        tone(27,2048,100,0);
        if(szMQTTData.length()>0 && szMQTTData.indexOf("https")>=0)
        {
